...
class CodeClippy(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            datasets.SplitGenerator(
                ...
                gen_kwargs={"filepaths": sorted([str(fp) for fp in Path(f"{data_dir}/train").rglob("*.json")])}
            ),
            datasets.SplitGenerator(
                name=datasets.Split.TEST,
                gen_kwargs={"filepaths": sorted([str(fp) for fp in Path(f"{data_dir}/test").rglob("*.json")])}
            ),
            datasets.SplitGenerator(
                name=datasets.Split.VALIDATION,
                gen_kwargs={"filepaths": sorted([str(fp) for fp in Path(f"{data_dir}/validation").rglob("*.json")])}
            ),
        ]
    ...
    def _generate_examples(self, filepaths:List):
        ...
        for filepath in filepaths:
            ...
            yield id_, {"id": id_, "text": theorem["env"] + "\n" + theorem["lemma"] + "\n" + theorem["proof"], \
                "file_name":filepath, "repo_name":"", "repo_language":"coq", "stars":"100", "mime_type":"text\/plain"}
