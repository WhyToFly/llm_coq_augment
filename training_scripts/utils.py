    ...
    def run(self):
        ...
        while True and i < self.max_steps:
            ...
            while l < max_length:
                ...
                sample = {k:sample[k]+next_sample[k] if type(next_sample[k]) is list else sample[k]+[next_sample[k]] for k in next_sample.keys()}
...
class PrefetchDataloader(multiprocessing.Process):
    ...
    def make_iter(self):
        if self.shuffle:
            shuffled_dataset = self.dataset.shuffle(buffer_size=self.shuffle_buffer, seed=self.seed)
            self.seed += 1
            self.ds_iter = iter(shuffled_dataset)
        else:
            self.ds_iter = iter(self.dataset)
